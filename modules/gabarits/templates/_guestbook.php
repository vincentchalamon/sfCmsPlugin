<?php $form = new GuestbookForm(null, array('front' => true)) ?>
<?php use_javascripts_for_form($form) ?>
<?php use_stylesheets_for_form($form) ?>
<?php $pager = new sfDoctrinePager("Guestbook", sfConfig::get('app_max_per_page', 20)) ?>
<?php $pager->setPage($sf_request->getParameter('page')) ?>
<?php $pager->setTableMethod("findActive") ?>
<?php $pager->init() ?>
<!-- Start main section -->
<section class="span-800">
  ...
  <div id="guestbookForm">
    <form action="<?php echo url_for("@guestbook_form") ?>" method="post" id="guestbook" class="ajaxForm">
      <fieldset>
        <legend>Déposer un message</legend>
        ...
        <?php echo $form["contents"]->renderLabel() ?>
  ...
  <?php if($pager->getNbResults()): ?>
    <div id="guestbookList">
      <?php foreach($pager->getResults() as $guestbook): ?>
        <div class="message">
          <span class="title">
            <?php echo $guestbook->getName() ?><span>le <?php echo format_date(strtotime($guestbook->getUpdatedAt()), "EEEE dd MMMM yyyy", $sf_user->getCulture()) ?></span>
          </span>
          <p><?php echo $guestbook->getContents() ?></p>
        </div>
      <?php endforeach ?>
      <?php if($pager->haveToPaginate()): ?>
        <?php include_partial('sfCms/pagination', array('pager' => $pager, 'url' => "sfCms/view?slug=".$sf_request->getParameter('slug'))) ?>
      <?php endif ?>
    </div>
  <?php endif ?>
</section>
