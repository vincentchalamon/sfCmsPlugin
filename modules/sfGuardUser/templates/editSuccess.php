...
<?php $sf_response->addMeta('title', 'Administration | '.__('Utilisateurs', array(), 'messages').' | '.__('Edition de "%%username%%"', array('%%username%%' => $sf_guard_user->getUsername()), 'messages')) ?>
<?php slot('breadcrumb', array(array('url' => '@sf_guard_user', 'label' => __('Utilisateurs', array(), 'messages')), array('url' => '@sf_guard_user_edit?id='.$sf_request->getParameter('id'), 'label' => __('Edition de "%%username%%"', array('%%username%%' => $sf_guard_user->getUsername()), 'messages')))) ?>
<?php include_partial('sfGuardUser/assets') ?>
<?php include_partial('sfGuardUser/flashes') ?>

<div class="contentcontainer">
  <div class="headings">
    <h2><?php echo __('Edition de "%%username%%"', array('%%username%%' => $sf_guard_user->getUsername()), 'messages') ?></h2>
  </div>
