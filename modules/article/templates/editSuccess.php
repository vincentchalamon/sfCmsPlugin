...
<?php $sf_response->addMeta('title', 'Administration | '.__('Articles', array(), 'messages').' | '.__('Edition de "%%title%%"', array('%%title%%' => $article->getTitle()), 'messages')) ?>
<?php slot('breadcrumb', array(array('url' => '@article', 'label' => __('Articles', array(), 'messages')), array('url' => '@article_edit?id='.$sf_request->getParameter('id'), 'label' => __('Edition de "%%title%%"', array('%%title%%' => $article->getTitle()), 'messages')))) ?>
    ...
    <?php echo form_tag_for($form, '@article') ?>
      ...
      <div class="left">
        <?php if($article->isSystem()): ?>
          <?php include_partial('article/list_header') ?>
        <?php endif ?>
